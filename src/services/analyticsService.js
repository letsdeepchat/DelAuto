...
class AnalyticsService {
  ...
  async getFailedDeliveryReductionMetrics(filters = {}) {
    ...
    try {
      ...
      const now = new Date();
      const currentPeriod = {
        startDate: filters.startDate || new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000).toISOString(),
        endDate: filters.endDate || now.toISOString()
      };

      const previousPeriod = {
        startDate: new Date(new Date(currentPeriod.startDate).getTime() - 30 * 24 * 60 * 60 * 1000).toISOString(),
      ...
      const metrics = {
        ...
        improvement: {
          failureRateReduction: Math.max(0, (previousStats.failureRate || 0) - (currentStats.failureRate || 0)),
          absoluteReduction: Math.max(0, (previousStats.failedDeliveries || 0) - (currentStats.failedDeliveries || 0)),
          percentageImprovement: previousStats.failureRate ?
            (((previousStats.failureRate - (currentStats.failureRate || 0)) / previousStats.failureRate) * 100) : 0
        },
  ...
  async getCustomerResponsePatterns(filters = {}) {
    ...
    try {
          ...
          $group: {
            ...
            dayBreakdown: {
              $push: {
                ...
                successRate: {
                  $multiply: [
                    { $divide: ['$successfulResponses', { $max: ['$count', 1] }] },
                    100
                  ]
                }
              }
      ...
      const hourlyPatterns = await mongoose.connection.db.collection('call_logs').aggregate(pipeline).toArray();
      ...
      const responseTypes = await mongoose.connection.db.collection('call_logs').aggregate(responseTypePipeline).toArray();
      ...
      const patterns = {
        ...
        recommendations: this.generateResponseRecommendations(hourlyPatterns, responseTypes[0]),
        generatedAt: new Date().toISOString()
      };
  ...
  async getAgentListeningCompliance(filters = {}) {
    ...
    try {
      ...
      const pipeline = [
        ...
        { $unwind: { path: '$agent', preserveNullAndEmptyArrays: true } },
        {
          $group: {
            ...
            totalDeliveries: { $sum: 1 },
            recordingsListened: {
              $sum: { $cond: [{ $eq: ['$listened_status', 'listened'] }, 1, 0] }
            },
            recordingsSkipped: {
        ...
        },
        {
          $project: {
            ...
            complianceRate: {
              $multiply: [
                { $divide: ['$recordingsListened', { $max: ['$totalDeliveries', 1] }] },
                100
              ]
            },
            ...
            successRateAfterListening: {
              $multiply: [
                { $divide: ['$successfulDeliveries', { $max: ['$recordingsListened', 1] }] },
                100
              ]
            }
          }
      ...
      const complianceData = await mongoose.connection.db.collection('recording_listens').aggregate(pipeline).toArray();

      // Overall compliance stats
      const overallStats = complianceData.length > 0 ? {
        totalAgents: complianceData.length,
        avgComplianceRate: complianceData.reduce((sum, agent) => sum + agent.complianceRate, 0) / complianceData.length,
        topPerformer: complianceData[0],
        needsImprovement: complianceData.filter(agent => agent.complianceRate < 80)
      } : {
        totalAgents: 0,
        avgComplianceRate: 0,
        topPerformer: null,
  ...
  async getROIFromDeliveryAutomation(filters = {}) {
    ...
    try {
      // Get failed delivery reduction metrics
      const reductionMetrics = await this.getFailedDeliveryReductionMetrics(filters);

      // Cost assumptions (customizable)
      const costs = {
        failedDeliveryCost: 25, // Cost of a failed delivery (redelivery, customer service, etc.)
        aiProcessingCost: 0.10, // Cost per AI analysis
      ...
      const callCosts = costs.monthlyVolume * costs.callCost * costs.avgCallDuration;
  ...
  calculateOverallResponseRate(hourlyPatterns) {
    ...
    const totalCalls = hourlyPatterns.reduce((sum, hour) => sum + hour.totalCalls, 0);
    const successfulCalls = hourlyPatterns.reduce((sum, hour) => sum + hour.successfulCalls, 0);
  ...
  generateResponseRecommendations(hourlyPatterns, responseTypes) {
    ...
    if (hourlyPatterns && hourlyPatterns.length > 0) {
      ...
      if (bestHour) {
        recommendations.push(`Schedule more calls during ${bestHour.hour}:00 for higher response rates`);
      }
    }

    if (responseTypes && responseTypes.breakdown) {
      const noAnswerRate = responseTypes.breakdown.find(r => r.status === 'no-answer')?.percentage || 0;
      if (noAnswerRate > 30) {
        recommendations.push('High no-answer rate detected. Consider implementing call-back scheduling.');
      }
    }
  ...
  calculateComplianceImpact(complianceData) {
    ...
    const highCompliance = complianceData.filter(agent => agent.complianceRate >= 90);
    const lowCompliance = complianceData.filter(agent => agent.complianceRate < 90);

    const highSuccess = highCompliance.length > 0 ?
      highCompliance.reduce((sum, agent) => sum + agent.successRateAfterListening, 0) / highCompliance.length : 0;

    const lowSuccess = lowCompliance.length > 0 ?
      lowCompliance.reduce((sum, agent) => sum + agent.successRateAfterListening, 0) / lowCompliance.length : 0;
